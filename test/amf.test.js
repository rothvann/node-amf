...
describe('AMF3', () => {
    ...
    test('Integer', () => {
        ...
        for(let i = 0; i < 4; i++) {
            let res = testDecoder(binary[i]);
            expect(res).toEqual(result[i]);
        }
    });
    
    test('Double', () => {
        let amf3 = '11 05 40 c8 1c d6 e6 31 f8 a1'.replace(/\s/g, '');
        let res = testDecoder(amf3);
        expect(res).toEqual(12345.6789);
    });
    
    test('String', () => {
        let string = Buffer.from('dynamic', 'utf8');
        let length = Buffer.from([string.length * 2 + 1], 'hex');
        let amf3 = Buffer.from('1106', 'hex');
        amf3 = Buffer.concat([amf3, length, string]);
        let res = decoder.decode(amf3);
    ...
    test('XMLDocument', () => {
        ...
        let xml = Buffer.from(xmlString, 'utf8');
        let length = Buffer.from([0b10001010, 0b00111111]);
    ...
    test('Object', () => {
        let amf3 = Buffer.from([0x11, 0x0a, 0b00101011], 'hex');
        ...
        let res = decoder.decode(Buffer.concat([amf3, members, dynamic]));
        expect(res).toEqual({test: 'test amf3 string', date: new Date(1588126908987), dynamic: 'dynamic string', name: ''});
    ...
    test('Byte Array', () => {
        let amf3 = Buffer.from([0x11, 0x0c, 0b00010101, 0x11, 0x21, 0x31, 0x41, 0x51, 0x61, 0x71, 0x81, 0x91, 0x10], 'hex');
        let res = decoder.decode(amf3);
